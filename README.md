...
报告由“能力域总结”和“逐项检测结果”两张表组成。逐项状态与结论全部来自 Skill 语义复核；点击任意检测项可展开检测目的、检测方法、通过条件、完整脱敏请求输入和请求输出。
...
- 单次请求默认超时为 90 秒，可通过 `--timeout SECONDS` 覆盖。8K/16K 长输出同时检查可见输出 Token、可见正文字节、完成标记、Result 字段、三个调查阶段、六个证据及其引用；慢模型仍建议使用 `--timeout 300`。
