...
当前 v0.5.0 脚本包含 62 个连续编号的核心检测项，不在采集脚本中保留旧版 113 项目录；报告 Skill 仍能读取旧版完整日志，并保留其中发现的全部历史检测项：

| ID | 能力范围 | 检测重点 |
| --- | --- | --- |
| `001-008` | 接口与协议 | URL、协议识别、鉴权、模型名称、同步/流式、usage、错误正文 |
| `009-013` | 结构化结果 | 裸 JSON、字段类型、嵌套结构、Result 核心字段、阶段与证据引用 |
| `014-018` | 上下文容量 | 8K/16K/32K/64K/128K 级字符近似输入、尾部标记召回和观测输入 Token |
| `019-025` | 指令与文本 | 精确输出、组合格式、修正优先级、抽取、分类、摘要、去重 |
| `026-031` | 上下文行为 | 首中尾召回、跨段关联、相似干扰、多轮修正 |
...
| `051-058` | 性能与稳定性 | 总延迟、流式 TTFB、重复成功率、P50/P95、8 并发、持续请求与恢复探针 |
...
| `UNDETERMINED` | 证据缺失、矛盾、截断，或当前协议下无法安全解释 |
...
- 上下文容量档位使用字符负载近似：约 32K、64K、128K、256K、512K 字符分别代表 8K、16K、32K、64K、128K 级输入压力，不等同于供应商 tokenizer 的精确 Token 数。报告会把响应中的观测输入 Token 与字符数分开呈现。
- 容量测试采用大输入和短标记回答；单次成功只证明该次请求的可观察下限，不证明模型的最大上下文窗口，也不检测长输出能力。
- 检测项 `053` 记录 curl `time_starttransfer`，即流式请求的 TTFB；它不是首 Token 时间（TTFT），代理缓冲也可能影响该值。
- 检测项 `058` 先执行 10 次连续请求，再发送一个内容与标记均独立的恢复探针；它不代表并发负载，并发能力由 `057` 单独观察。
- P50/P95 来自 5 次真实成功请求的端到端延迟排序，是本次运行快照，不是服务 SLA。
- 单次请求默认超时为 120 秒，可通过 `--timeout SECONDS` 覆盖。
