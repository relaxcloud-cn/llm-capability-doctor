...
pub static RULES: &[Rule] = &[
    ...
    Rule {
        ...
        effect: "LLM模型网关无法生成 Codex 工具调用事件。",
    },
    Rule {
        id: "GW-ANTH-SHAPE-001",
    ...
    Rule {
        id: "GW-GOOGLE-TOOL-004",
        adapter: Adapter::Google,
        requirement: "Google functionCall 的 args 必须是对象。",
        source_file: "src/adapters/google.ts",
        source_test: "tests/google-adapter.test.ts",
        effect: "LLM模型网关无法生成有效的函数调用。",
    },
];
...
pub fn rule(id: &str) -> &'static Rule {
    RULES
        ...
        .expect("LLM gateway evaluator only references declared rules")
}
