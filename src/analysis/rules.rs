...
pub const RULES: [AnalysisRule; 42] = [
    ...
    rule!(
        "018",
        "上下文沿用原有逻辑：成功容量探针达到 124000 Token 即满足 128K 要求（保留回复余量）；优先使用服务端 prompt tokens，缺失 usage 时使用构造值并注明估计来源。探针因超长被拒且搜索出的上限不足 124000 为 FAIL；超时、疑似截断或与长度无关的错误记为无法判断。最终结论由本地共用容量判定函数核定。"
    ),
    ...
    rule!(
        "057",
        "检测 4、8、16 并发，记录各波次失败请求数量和全部请求的平均响应时间。请求成功要求传输完成、HTTP 2xx、协议有效且模型回复非空。4 并发必须 4/4 成功且平均响应时间不超过 30 秒；任一失败或平均时间超过 30 秒即 FAIL，缺失计时不能判通过。8、16 并发仅作参考，不影响 057 总体结果。"
    ),
    rule!(
        "059",
