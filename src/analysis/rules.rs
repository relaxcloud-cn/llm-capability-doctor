...
pub const RULES: [AnalysisRule; 42] = [
    ...
    rule!(
        "018",
        "刚性要求：128K = 128000 Token，不设 124K 容差。成功容量探针的服务端 prompt tokens 不低于 128000 才能证明通过；确认上下文上限低于 128000 为 FAIL。缺失 usage、超时、疑似截断或区间跨越门槛时证据不足，不得宣称通过；最终容量结论由本地证据规则核定。"
    ),
    ...
    rule!(
        "057",
        "本波次每个请求都必须传输完成、HTTP 2xx、协议有效且模型回复非空；任一请求失败则本波次 FAIL。057 总体刚性门槛仅使用 4 并发波次，必须 4/4 成功；8、16、32 并发只作性能参考，不影响总体通过。响应延迟不设通过上限。"
    ),
    rule!(
        "059",
